import re
import pandas as pd
import numpy as np

from functools import reduce

from os.path import join as pjoin

ROOT_DIR = '..'
DATA_DIR = pjoin(ROOT_DIR, 'data')

def del_newline(text : str):
    return re.sub('[\s\n\t]+', ' ', text)

def del_special_char(text : str):
    return re.sub('[^가-힣ㄱ-ㅎㅏ-ㅣ,.?!~@0-9a-zA-Z\s]+', '', text)

def del_nickname(text : str):
    return re.sub('@[가-힣ㄱ-ㅎㅏ-ㅣA-Za-z0-9\.\?]*', '', text)

def preprocess(text : str):
    proc_txt = del_newline(text)
    proc_txt = del_special_char(proc_txt)
    proc_txt = del_nickname(proc_txt)

    return proc_txt.strip()

def is_valid(proc_text : str, threshold=2) -> bool:
    return len(proc_text) > threshold

if __name__=="__main__":
    train = pd.read_csv(pjoin(DATA_DIR, 'train.csv')).dropna(axis=0)
    test = pd.read_csv(pjoin(DATA_DIR, 'test.csv')).dropna(axis=0)
    valid = pd.read_csv(pjoin(DATA_DIR, 'valid.csv')).dropna(axis=0)

    # print(f"Number of Data\n\tTrain: {len(train)}\n\tTest: {len(test)}\n\tValid: {len(valid)}")


    data = pd.concat([train, test, valid], ignore_index=True)
    data = data.sample(frac=1)

    data['proc_text'] = list(map(preprocess, data['sentence']))
    data.to_csv(pjoin(DATA_DIR, 'data.csv'), index=False)
    
    valid_ratio = 0.15
    num_drop = 0

    valid = pd.DataFrame()
    test = pd.DataFrame()
    train = pd.DataFrame()


    hate_data = data[data.label==0]
    num_valid = int(len(hate_data) * valid_ratio)
    valid = pd.concat([valid, hate_data.iloc[:num_valid]], ignore_index=True)
    test = pd.concat([test, hate_data.iloc[num_valid:2*num_valid]], ignore_index=True)
    train = pd.concat([train, hate_data.iloc[2*num_valid:]], ignore_index=True)

    normal_data = data[data.label==1]
    num_valid = int(len(normal_data) * valid_ratio)
    valid = pd.concat([valid, normal_data.iloc[:num_valid]], ignore_index=True)
    test = pd.concat([test, normal_data.iloc[num_valid:2*num_valid]], ignore_index=True)
    train = pd.concat([train, normal_data.iloc[2*num_valid:]], ignore_index=True)
    
    valid = valid.sample(frac=1)
    test = test.sample(frac=1)
    train = train.sample(frac=1)

    valid.to_csv(pjoin(DATA_DIR, 'valid.csv'), index=False)
    test.to_csv(pjoin(DATA_DIR, 'test.csv'), index=False)
    train.to_csv(pjoin(DATA_DIR, 'train.csv'), index=False)
    # print(f"Number of Data\n\tTrain: {len(train)}\n\tTest: {len(test)}\n\tValid: {len(valid)}\n\n")

    print(f"Total Number of Data : {len(data)} -> {len(valid) + len(test) + len(train)}")
    print(f"Drop num: {num_drop} \nLabel num: {len(valid.label.unique())}")
